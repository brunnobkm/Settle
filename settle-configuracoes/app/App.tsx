...
function Configuracoes() {
  ...
  useEffect(() => {
    document.title =
      rota === "inicio"
        ? "Settle · Configurações"
        : `Settle · ${nome} · Configurações`;
  }, [rota, nome]);
    ...
    ...(isAdmin
      ? [
          ...
          },
          {
            label: "Organização",
            items: [
              item("equipe", UsersIcon),
              item("permissoes", ShieldIcon),
              item("auditoria", ClockIcon),
            ],
          },
          {
            label: "Inteligência",
  ...
  const Pagina =
    !isAdmin && SO_ADMIN.includes(rota) ? SemPermissao : PAGINAS[rota];

  return (
    <ChatProvider rota={rota}>
      {/* A conversa lateral é coluna do layout, como a sidebar: empurra a tela em vez de cobrir. */}
      <div className="flex">
