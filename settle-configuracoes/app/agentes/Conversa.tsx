...
function CartaoDaPergunta({
  ...
  return (
    <section aria-labelledby={`pergunta-${passo.id}`} className="rounded-xl border bg-card shadow-xs">
      <div className="flex items-start gap-3 px-4 pt-3.5 pb-2">
        <div className="min-w-0 flex-1">
          <h3 id={`pergunta-${passo.id}`} className="text-sm leading-5 font-semibold">
            {passo.pergunta}
          </h3>
          {passo.ajuda && <p className="mt-0.5 text-[12.5px] leading-[18px] text-muted-foreground">{passo.ajuda}</p>}
        </div>
        <span className="shrink-0 text-xs text-muted-foreground tabular-nums">
          {n} de {total}
        </span>
      </div>
      <ul role="radiogroup" aria-labelledby={`pergunta-${passo.id}`} className="flex flex-col px-2">
        {passo.opcoes.map((o) => (
...
export function ConversaAgente() {
  ...
  const [falas, setFalas] = useState<Fala[]>([
    {
      ...
      texto: (
        <>
          Vou te ajudar a criar um agente com algumas perguntas. Ao lado, você vê o agente sendo montado. Se preferir, use{" "}
          <b>Configurar manualmente</b> a qualquer momento: o que já respondeu vai junto.
        </>
      ),
    },
  ])
  ...
  const registrar = (pergunta: string, resposta: ReactNode, eco?: ReactNode) =>
    setFalas((f) => [...f, { quem: "settle", texto: pergunta }, { quem: "voce", texto: resposta }, ...(eco ? [{ quem: "settle" as const, texto: eco }] : [])])
  ...
  const responder = (valor: string, rotulo: string) => {
    ...
    if (id === "quando") {
      ...
      if (livre) eco = <>Não tenho esse momento na lista; deixei <b>{GATILHOS[g]}</b>. Você pode trocar na revisão.</>
      const novoRep = !!REP_POR_GATILHO[g]
      setR((x) => ({ ...x, gatilho: g }))
      registrar(passo.pergunta, rotulo, eco)
    ...
    if (id === "repete") setR((x) => ({ ...x, repete: (livre ? "sempre" : valor) as Repeticao }))
    if (id === "onde") {
      const o = (livre ? ondeProvavel(livre) ?? "habilitacao" : valor) as Onde
      if (livre) eco = <>Entendi como <b>{ONDE[o].t}</b>. Você pode trocar na revisão.</>
      setR((x) => ({ ...x, onde: o }))
    }
    if (id === "aprovacao") setR((x) => ({ ...x, aprovacao: (livre ? "manual" : valor) as ModoAprovacao }))
    registrar(passo.pergunta, rotulo, eco)
    proximo(id)
  }
  ...
  const pular = () => {
    if (!passo) return
    setFalas((f) => [...f, { quem: "settle", texto: passo.pergunta }, { quem: "voce", texto: <span className="text-muted-foreground">Pulei</span> }])
    proximo(id)
  }
      ...
      fim={
        <div className="flex flex-col gap-3 rounded-xl border bg-card p-4">
          <p className="text-[13.5px] leading-5">
            Pronto. O agente está montado ao lado. Revise os campos e crie: nada é criado antes disso.
          </p>
...
export function ConversaVariavel() {
  ...
  const [falas, setFalas] = useState<Fala[]>([
    {
      ...
      texto: (
        <>
          Vamos criar uma variável: uma pergunta que a Settle vai fazer a todo edital. Ao lado, você vê a variável sendo
          montada. <b>Configurar manualmente</b> leva o que já respondeu para o formulário.
        </>
      ),
    },
  ])
  ...
  const responder = (valor: string, rotulo: string) => {
    ...
    setFalas((f) => [...f, { quem: "settle", texto: passo.pergunta }, { quem: "voce", texto: rotulo }])
    proximo()
  }
  ...
  return (
    <Conversa
      ...
      fim={
        <div className="flex flex-col gap-3 rounded-xl border bg-card p-4">
          <p className="text-[13.5px] leading-5">
            Pronto. A variável está montada ao lado. Revise e crie: depois, use-a no que um agente faz.
          </p>
          <div className="flex gap-2">
            <Button onClick={revisar}>Revisar e criar</Button>
          </div>
