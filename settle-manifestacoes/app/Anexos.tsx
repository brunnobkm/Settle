...
// visualizar/baixar), seção de anexos do card e o menu "+N".
...
/** Botão só com ícone, com nome acessível e tooltip. */
 ...
 * - linha: com borda, ocupa a largura toda (modais)
 * - menu: sem borda, para listas dentro de menus
 */
export function LinhaDeAnexo({
  ...
  return (
    ...
    >
      ...
      <AttachmentContent className={className}>
  ...
  return (
    <div className="mt-3 border-t pt-3">
      <div className="mb-2 flex items-center gap-2">
        <span className="text-xs font-bold text-foreground">Anexos</span>
        {/* "Baixar todos" só faz sentido com mais de 1 anexo */}
        {anexos.length > 1 && (
          <Button size="xs" className="ml-auto px-2.5" data-nao-prototipado>
            Baixar todos
          </Button>
        )}
      </div>
      <div className="flex items-stretch gap-2">
        ...
        {resto.length > 0 && (
          <MaisAnexos anexos={resto} deslocamento={MAX_VISIVEIS} onVisualizar={onVisualizar} />
        )}
      </div>
    </div>
  )
    ...
    <Popover>
      ...
      <PopoverContent align="end" className="w-85 max-w-[86vw] gap-0 rounded-xl p-1.5" onClick={(e) => e.stopPropagation()}>
        <div className="mb-1 flex items-center justify-between gap-2 border-b px-1.5 pt-1 pb-2">
          ...
          <Button variant="ghost" size="xs" className="text-primary hover:text-primary" data-nao-prototipado>
        ...
        <div className="flex max-h-65 flex-col gap-0.5 overflow-y-auto">
          {anexos.map((anexo, i) => (
            <LinhaDeAnexo
              ...
              anexo={anexo}
              forma="menu"
              onVisualizar={() => onVisualizar(i + deslocamento)}
            />
          ))}
        </div>
      </PopoverContent>
...
/** Lista vertical de arquivos (modais). */
