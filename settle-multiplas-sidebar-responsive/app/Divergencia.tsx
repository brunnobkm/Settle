// Divergência: a mesma informação veio diferente de documentos distintos. "Revisar" mostra as
// opções para escolher (ou adicionar uma nova informação, com trecho do edital opcional);
// "Confirmar" resolve e a escolhida vira uma informação da seção.
...
export function Divergencia({ painel, id, semBordaSuperior }: { painel: Painel; id: string; semBordaSuperior?: boolean }) {
  ...
  return (
    <Collapsible open={d.aberta} onOpenChange={() => painel.alternarDivergencia(id)}>
      <div
        className={cn(
          "grid items-center gap-2 border-y px-4 py-2.5 transition-colors hover:bg-muted max-[520px]:gap-2.5",
            ...
            >
              <div className={cn("grid gap-2", d.revisando && "grid-cols-[18px_minmax(0,1fr)] items-start")}>
                ...
                <label
                  htmlFor={`${radio}-${i}`}
                  className={cn(
                    "block min-w-0 text-[13px] leading-[1.45] font-medium text-muted-foreground",
                    d.revisando && "cursor-pointer"
                  )}
                >
                  {opcao.texto}
                </label>
