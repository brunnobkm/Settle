// Histórico de divergências (ícone de alerta no cabeçalho ou item do menu "Mais ações"):
// pendentes primeiro, depois as resolvidas nesta sessão e o histórico de exemplo, até 7 itens.
        ...
        {opcoes.map((opcao, i) => {
          ...
          return (
            <li key={i} className="grid grid-cols-[18px_minmax(0,1fr)] items-start gap-2.25 p-2.5">
              <Icone
                aria-hidden
                className={cn("size-4.5", escolhida ? "text-success" : resolvida ? "text-muted-foreground" : "text-warning")}
              />
              <div>
                ...
                <p className={cn("mt-1 text-xs leading-snug text-muted-foreground", escolhida && "font-semibold text-foreground/80")}>
                  {opcao}
                </p>
              </div>
            </li>
...
/** Conteúdo do popover; o gatilho e a âncora ficam no cabeçalho do workspace. */
export function ConteudoDoHistorico({ painel }: { painel: Painel }) {
  return (
    <PopoverContent
      align="end"
      aria-label="Histórico de divergências"
