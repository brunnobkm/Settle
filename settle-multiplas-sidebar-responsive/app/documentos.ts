// Documentos da licitação (JetBrains All Products Pack, TRF6) abertos em "Arquivos da licitação".
// Cada página traz os parágrafos do documento; as linhas marcadas com TITULO/SUBTITULO são os
// títulos de seção e subseção do edital. Parágrafos de preenchimento completam cada página até
// ~1.750 caracteres (mesma regra do protótipo original).

export type DocKey = "edital" | "termo" | "etp" | "contrato" | "proposta"

/** Marcas de título de seção e de subseção no início do parágrafo. */
export const MARCA_TITULO = "@@TITULO@@"
export const MARCA_SUBTITULO = "@@SUBTITULO@@"

const DOCUMENTOS: Record<DocKey, { rotulo: string; paginas: string[][] }> = {
  edital: {
    rotulo: "Edital 15/2023",
    paginas: [
      [
  ...
  etp: {
    rotulo: "Estudo Técnico Preliminar",
    paginas: [
      [
...
const PREENCHIMENTO: Record<DocKey, string[]> = {
  edital: [
...
const ehTitulo = (paragrafo: string) =>
  paragrafo.startsWith(MARCA_TITULO) || paragrafo.startsWith(MARCA_SUBTITULO)

/** Completa a página com parágrafos de preenchimento até ~1.750 caracteres (no máximo 8 parágrafos de texto). */
function montarPagina(documento: DocKey, paragrafos: string[], indice: number) {
  const preenchimento = PREENCHIMENTO[documento]
  const pagina = [...paragrafos]
  const textosDaBase = paragrafos.filter((p) => !ehTitulo(p)).length
  let textos = textosDaBase
  let cursor = indice % preenchimento.length
  let caracteres = paragrafos.reduce((total, p) => (ehTitulo(p) ? total : total + p.length), 0)
  const ALVO = 1750
  const MAXIMO = 2100

  while (caracteres < ALVO && textos < 8) {
    const proximo = preenchimento[cursor]
    if (caracteres + proximo.length > MAXIMO && textos > textosDaBase) break
    pagina.push(proximo)
    caracteres += proximo.length
    textos += 1
    cursor = (cursor + 1) % preenchimento.length
  }
  return pagina
}

/** Tira a marca de título: o visualizador mostra os títulos como parágrafos. */
const semMarca = (paragrafo: string) => paragrafo.replace(MARCA_TITULO, "").replace(MARCA_SUBTITULO, "")

export const LISTA_DE_DOCUMENTOS = (Object.keys(DOCUMENTOS) as DocKey[]).map((value) => ({
  value,
  label: DOCUMENTOS[value].rotulo,
}))

/** Páginas prontas de cada documento (lista de parágrafos por página). */
export const PAGINAS = Object.fromEntries(
  (Object.keys(DOCUMENTOS) as DocKey[]).map((chave) => [
    chave,
    DOCUMENTOS[chave].paginas.map((pagina, i) => montarPagina(chave, pagina, i).map(semMarca)),
  ])
